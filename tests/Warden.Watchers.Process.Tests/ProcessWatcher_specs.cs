...
namespace Warden.Tests.Watchers.Process
{
    ...
    public class when_invoking_execute_async_method_with_valid_configuration : ProcessWatcher_specs
    {
        ...
        It should_have_set_values_in_process_check_result = () =>
        {
            ...
            ProcessCheckResult.ProcessInfo.Id.Should().Be(ProcessId);
            ProcessCheckResult.ProcessInfo.Name.Should().Be(ProcessName);
            ProcessCheckResult.ProcessInfo.Exists.Should().Be(ProcessExists);
            ProcessCheckResult.ProcessInfo.Responding.Should().Be(ProcessResponding);
        };
    }
    ...
    public class when_invoking_ensure_predicate_that_is_invalid : ProcessWatcher_specs
    {
        ...
        It should_have_set_values_in_process_check_result = () =>
        {
            ...
            ProcessCheckResult.ProcessInfo.Id.Should().Be(ProcessId);
            ProcessCheckResult.ProcessInfo.Name.Should().Be(ProcessName);
            ProcessCheckResult.ProcessInfo.Exists.Should().Be(false);
            ProcessCheckResult.ProcessInfo.Responding.Should().Be(false);
        };
    }
